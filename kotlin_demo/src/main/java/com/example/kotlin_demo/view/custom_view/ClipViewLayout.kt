 ...
 * 头像上传原图裁剪容器
 */
class ClipViewLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet = null!!, defStyleAttr: Int = 0) : RelativeLayout(context, attrs, defStyleAttr) {
    //裁剪原图
    private var imageView: ImageView? = null
    //裁剪框
    private var clipView: ClipView? = null
    ...
    fun init(context: Context, attrs: AttributeSet) {
        ...
        mHorizontalPadding = array.getDimensionPixelSize(R.styleable.ClipViewLayout_mHorizontalPadding,
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50f, resources.displayMetrics).toInt()).toFloat()
        //获取裁剪框边框宽度，默认1dp
    ...
    fun initSrcPic(uri: Uri?) {
        ...
        val path = getRealFilePathFromUri(context, uri)
    ...
    companion object {
        ...
        fun zoomBitmap(bitmap: Bitmap, w: Int, h: Int): Bitmap {
            ...
            val newBmp = Bitmap.createBitmap(bitmap, 0, 0, width, height, matrix, false)
            return newBmp
        }
